...
TILE_TEXTURES = [
    ...
    # the wormhole that sits outside the tunnel (the Space backdrop's Wormhole
    # model samples singledpi_texture_wormhole): the swirling disc is drawn
    # between the distant tunnels and the player's own tube
    ("tex_wormhole",    "images/singledpi_texture_wormhole.png"),
]
...
with open(OUT, "w") as f:
    f.write("/* assets_data.h - tile textures + character index.\n")
    f.write(" * Per-character frames live in char_<name>.h (split keeps files small).\n")
    ...
    mf = f  # main index handle; the loop body below writes char data via rebound f
    # Process character sprite sheets (one header per character)
    for char_id, char_name, png_rel, json_rel, anim_set in CHARS:
        ...
        # Per-character header (keeps every generated file well under limits)
        char_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                 f"char_{char_name}.h")
        cf = open(char_path, "w")
        cf.write(f"/* char_{char_name}.h - baked frames for {char_name}.\n")
        cf.write(" * Generated by bake_assets.py. Do not edit manually. */\n")
        cf.write(f"#ifndef CHAR_{char_name.upper()}_H\n#define CHAR_{char_name.upper()}_H\n\n")
        ...
        f.write(f"#endif /* CHAR_{char_name.upper()}_H */\n")
        cf.close()
    ...
    # ===== SKYBOX (star sphere) =====
    # The original puts a 6-face skybox cube (skybox0..5.png) around the
    # camera and renders it into a texture behind everything. Each face is a
    # pure point-starfield (98% of its texels are black), so baking every star
    # texel as a unit direction + brightness is both faithful and cheap: the
    # renderer projects one point per star with the camera's roll and pitch,
    # instead of cube-sampling a megapixel of sky per frame.
    #
    # Face order and cube axes come from the original's face-direction array
    # [-x, -y, +x, +z, -z, +y] (Space backdrop builds Skybox0..5 at dir*40).
    SKY_FACES = [
        (0, "-X"), (1, "-Y"), (2, "+X"), (3, "+Z"), (4, "-Z"), (5, "+Y"),
    ]
    SKY_DIR = {
        "-X": lambda s, t: (-1.0, -t,   s),
        "+X": lambda s, t: ( 1.0, -t,  -s),
        "-Y": lambda s, t: (   s, -1.0, -t),
        "+Y": lambda s, t: (   s,  1.0,  t),
        "+Z": lambda s, t: (   s, -t,   1.0),
        "-Z": lambda s, t: (  -s, -t,  -1.0),
    }
    SKY_THRESH = 96       # texel brightness that counts as a star
    SKY_SCALE = 30000.0   # int16 direction scale (dir*300 == raw*0.01)

    stars = []
    for face, axis in SKY_FACES:
        full = os.path.join(ASSETS_DIR, f"images/singledpi_texture_skybox{face}.png")
        if not os.path.exists(full):
            print(f"  skybox{face}: NOT FOUND")
            continue
        ...
        for v in range(h):
            ...
            for u in range(w):
                ...
                if b < SKY_THRESH:
                    continue
                s = (2.0 * (u + 0.5) / w) - 1.0
                dx, dy, dz = SKY_DIR[axis](s, t)
                ln = (dx * dx + dy * dy + dz * dz) ** 0.5
                stars.append((int(round(dx / ln * SKY_SCALE)),
                              int(round(dy / ln * SKY_SCALE)),
                              int(round(dz / ln * SKY_SCALE)), b))
                kept += 1
        print(f"  skybox{face} ({axis}): {kept} stars")

    sky_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "skybox_data.h")
    with open(sky_path, "w") as sf:
        sf.write("/* skybox_data.h - the baked star sphere.\n")
        sf.write(" * The original's Space backdrop renders a 6-face skybox cube\n")
        sf.write(" * (skybox0..5.png) behind the tunnel; each face is a point\n")
        sf.write(" * starfield, baked here as a unit direction + brightness per\n")
        sf.write(" * star. Generated by bake_assets.py. Do not edit manually. */\n")
        sf.write("#ifndef SKYBOX_DATA_H\n#define SKYBOX_DATA_H\n\n")
        sf.write("#include <stdint.h>\n\n")
        sf.write("/* dir components are unit * 30000, so dir * 300 == raw * 0.01 */\n")
        sf.write("typedef struct { int16_t x, y, z; uint8_t b, pad; } sky_star_t;\n\n")
        sf.write(f"#define SKY_STAR_COUNT {len(stars)}\n\n")
        sf.write("static const sky_star_t sky_stars[SKY_STAR_COUNT] = {\n")
        for i in range(0, len(stars), 6):
            row = "  " + " ".join("{%d,%d,%d,%d,0}," % s for s in stars[i:i + 6])
            sf.write(row + "\n")
        sf.write("};\n\n#endif /* SKYBOX_DATA_H */\n")
    print(f"  sky_stars: {len(stars)} total -> {sky_path}")
    f.write('/* baked star sphere (skybox faces) */\n#include "skybox_data.h"\n\n')

    # ===== CHARACTER INDEX TABLE =====
    # Generic C accessors for the WASM renderer
    f.write("/* per-character frame data (split headers) */\n")
    for cid, cname, _ in char_info:
        f.write(f'#include "char_{cname}.h"\n')
    f.write("\n")
